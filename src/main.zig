...
pub fn main() !void {
    ...
    const fileReader = blk: {
        if (std.mem.eql(u8, inputFilePath, "-")) {
            break :blk std.io.getStdIn().reader();
        } else {
            break :blk (try std.fs.cwd().openFile(inputFilePath, .{ .mode = .read_only })).reader();
        }
    };
    const fileWriter = blk: {
        if (std.mem.eql(u8, outputFilePath, "-")) {
            break :blk std.io.getStdOut().writer();
        } else {
            break :blk (try std.fs.cwd().createFile(outputFilePath, .{})).writer();
        }
    };
